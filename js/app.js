...
if (iconMenu != null) {
	...
	iconMenu.addEventListener("click", function(e) {
		if (unlock) {
			let toggleActive = document.querySelector('.main-menu__menu');
			let toggleActiveBody = document.querySelector('.menu__body');
			iconMenu.classList.toggle("_active");
			toggleActive.classList.toggle('_active');
			toggleActiveBody.classList.toggle('_active');
			body_lock(delay);
		}
	});
};
...
document.addEventListener("click", function(e) {
   if(!e.target.closest('.menu__body') && !e.target.closest('.icon-menu') && e.target.closest('.main-menu__menu._active')) {
      let removeActive = document.querySelector('.main-menu__menu');
      let removeActiveBody = document.querySelector('.menu__body');
			iconMenu.classList.remove("_active");
      removeActive.classList.remove('_active');
      removeActiveBody.classList.remove('_active');
      // body_lock(delay);
      body_lock();
   }
});
...
window.onload = function () {
	...
	function documentActions(e) {
		...
		if(isMobile.any()) {
			...
			if(targetElement.closest('.price-table__th-icon') && document.querySelectorAll('.price-table__th-icon._hover').length > 1) {
				_removeClasses(document.querySelectorAll('.price-table__th-icon._hover'), "_hover")
		 	}
		}
 }
